...
2. **Zero-Trust Backend Validation**:
...
3. **Google Forms & External Import Reconciliation (`mismatchDetector.js`)**:
...
4. **Immutable Audit Trail (`auditService.js`)**:
...
| Role | Name / Identifier | Password | Key Characteristics |
| :--- | :--- | :--- | :--- |
| **Placement Officer (Admin)** | `admin@placement.edu` (or `admin`) | `Admin@123` | Full administrative control, dynamic drive creator, student master DB, fraud & discrepancy center, Google Form reconciliation, audit trail. |
| **Student (Eligible)** | USN `1MS21CS001` (Aarav Patel) | `Student@123` | Verified CGPA: **8.65**, Active Backlogs: **0**, Branch: **CSE**, 2027 batch. Eligible for all tier-1 companies (ABC Tech, FinTech, Cloud). |
| **Student (Ineligible / Fraud Test)** | USN `1MS21CS042` (Rahul Sharma) | `Student@123` | Verified CGPA: **6.80**, Active Backlogs: **1**, Branch: **CSE**, 2027 batch. Ineligible for ABC Technologies (Min 7.50 CGPA). Use Google Form Sim to test submitting fake 8.20 CGPA! |
| **Recruiter** | `recruiter@abc-tech.com` | `Recruiter@123` | Recruiter partner portal for ABC Technologies. View certified eligible applicants, download resumes, and advance recruitment stages. |
...
The platform features a clean relational database architecture with zero-dependency native SQLite (`DatabaseSync`), configured via `schema.sql` and managed through dedicated CLI scripts:
...
### Database Schema
The database DDL is centralized in `backend/src/config/schema.sql`, defining all 11 relational tables, foreign key cascades, and query performance indexes:
...
4. **Initialize Database Schema & Create Administrator**:
   ```bash
   npm run db:setup
   ```
...
5. **(Optional) Import Student Academic Master Spreadsheet**:
...
2. Sign in using any of the evaluation credentials:
...
This verifies:
1. Primary Administrator authenticates.
2. Verified student `1MS21CS042` has certified CGPA = 6.80 and 1 backlog.
3. Live eligibility engine identifies student as ineligible for ABC Technologies (Min 7.50).
4. Student submits fake CGPA = 8.20 and backlogs = 0 via Google Form simulation.
5. Mismatch engine detects discrepancy and records entry in `data_mismatches`.
6. Malicious client attempts to apply with spoofed payload `{ cgpa: 8.20 }`.
7. Backend intercepts, ignores payload, uses certified 6.80, and rejects with `HTTP 403 Forbidden`.
8. Discrepancy is recorded in audit logs and visible on the admin dashboard.
...
1. **Create Production Database & Restricted User**:
   ```sql
...
3. **Database Maintenance & Automated Backups**:
...
1. **Create Production Environment File (`backend/.env`)**:
   ```env
   NODE_ENV=production
   PORT=5000
...
2. **Configure Nginx Site (`/etc/nginx/sites-available/veriplace`)**:
   ```nginx
   server {
       listen 80;
